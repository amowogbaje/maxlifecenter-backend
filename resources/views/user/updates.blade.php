    ...
    <div class="space-y-4">
        <div class="mb-6 lg:mb-8 overflow-x-auto">
            <div class="bg-blue-100 rounded-3xl p-0 inline-flex min-w-max">
                <a href="{{ route('user.updates') }}" class="bg-yellow-400 text-dashboard-white px-4 lg:px-8 py-2 lg:py-3 rounded-2xl font-bold text-sm lg:text-base transition-all whitespace-nowrap">
                    Updates
                </a>
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 lg:gap-6">
                            <div class="flex flex-col gap-1 min-w-0">
                                ...
                                <span class="text-base font-bold text-text-dark truncate" title="{{ $update['title'] }}">{{ $update['title'] }}</span>
                            </div>
                            <div class="flex flex-col gap-1 min-w-0">
                                <span class="text-sm text-text-light truncate">Point</span>
                                <span class="text-base font-bold text-text-dark truncate" title="{{ $update['bonus_point'] }}"><img src="{{ asset('images/icons/diamond.svg') }}" alt="Rewards" class="inline-block w-4 h-4 mr-1">{{ $update['bonus_point'] }}</span>
                            </div>
                            <div class="flex flex-col gap-1 min-w-0 sm:col-span-2 lg:col-span-1">
                                <span class="text-sm text-text-light">Date</span>
                                <span class="text-xs lg:text-base text-text-dark truncate" title="{{ $update['date'] }}">{{ $update['date'] }}</span>
                            </div>
                            @if(!empty($update['id']))
                            <div class="flex flex-col gap-1 min-w-0">
                                <span class="text-sm text-text-light truncate">ID</span>
                                <span class="text-base text-text-dark truncate" title="{{ $update['id'] }}">{{ $update['id'] }}</span>
                            </div>
                            @endif
                            <div class="flex flex-col gap-1 min-w-0">
                                <span class="text-sm text-text-light">Segment</span>
                                <span class="text-base text-text-dark truncate" title="{{ $update['segment'] }}">{{ $update['segment'] }}</span>
                            </div>
                        </div>
                    </div>
