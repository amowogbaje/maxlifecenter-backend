    ...
    <div class="space-y-4">
        <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-4 w-full lg:w-auto">
                <form method="GET" action="{{ route('admin.rewards')}}" class="flex flex-col lg:flex-row gap-4 w-full" id="filterForm">
                    ...
                    <!-- Tier Level Filter -->
                    <div class="relative flex-1 lg:w-auto">
                        <select name="tier_level" class="w-full h-12 pl-4 pr-10 bg-white rounded-[14px] shadow-sm border-0 text-text-gray appearance-none cursor-pointer" onchange="document.getElementById('filterForm').submit()">
                            <option value="">All Tiers</option>
                            @foreach($rewards as $reward)
                            <option value="{{ $reward->title }}" {{ request()->input('tier_level') == $reward->title ? 'selected' : '' }}>
                                {{ $reward->title }}
                            </option>
                            @endforeach
                        </select>
