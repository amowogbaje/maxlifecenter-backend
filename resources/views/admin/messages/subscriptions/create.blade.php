...
<div class="max-w-4xl mx-auto py-10">
    <div class="bg-white shadow-xl rounded-2xl p-8">
        ...
        <form action="{{ route('admin.messages.subscriptions.store') }}" method="POST" class="space-y-8">
            @csrf

            <!-- Title -->
            <div>
                <label class="block text-sm font-semibold text-gray-700">Title</label>
                <input type="text" name="title" class="mt-2 w-full rounded-xl border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 shadow-sm px-4 py-2" placeholder="Subscription List Title here" required>
            </div>
