...
<div class="p-4 lg:p-[32px] space-y-6 lg:space-y-8">
    ...
    <div class="space-y-4">
        <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            <h2 class="text-xl lg:text-2xl font-bold text-foreground">Role List</h2>
            <div x-data="{ openCreateModal: {{ $errors->any() ? 'true' : 'false' }} }" x-cloak>
                ...
                <div x-show="openCreateModal" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
                    ...
                    <!-- Modal -->
                    <div @click.outside="openCreateModal = false" class="bg-white rounded-2xl shadow-lg w-full max-w-lg p-6 relative">

                        <button @click="openCreateModal = false" class="absolute top-3 right-3 text-gray-400 hover:text-gray-600 text-2xl">&times;
                        </button>

                        <h2 class="text-lg font-bold mb-4 text-gray-800">Create New Role</h2>
                        ...
                        <form method="POST" action="{{ route('admin.roles.store') }}" class="space-y-5">
                            ...
                            <div>
                                ...
                                <input type="text" name="name" value="{{ old('name') }}" class="w-full border rounded-lg p-2.5 focus:ring focus:ring-blue-200" required>
                                @error('name')
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-3 gap-3 max-h-64 overflow-y-auto pr-1">
                                    ...
                                    <label class="flex items-center justify-between border rounded-lg p-3 hover:bg-gray-50 transition">
                                        ...
                                        <label class="relative inline-flex items-center cursor-pointer">
                                            ...
                                            <div class="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:after:translate-x-5 peer-checked:bg-blue-500 
                        after:content-[''] after:absolute after:top-[2px] after:left-[2px] 
                        after:bg-white after:border after:rounded-full after:h-5 after:w-5 
                        after:transition-all after:duration-200 border-gray-300">
                                            </div>
                                        </label>
                                    </label>
                                    @endforeach
        ...
        <div class="space-y-3" x-data="{
                showEditModal: false, 
                selectedRole: { id: null, name: '', permissions: [] }
            }" x-cloak>
            @if($roles->count() > 0)
            ...
            <div class="bg-white rounded-[24px] shadow-sm p-4 lg:p-5 overflow-hidden">
                <div class="flex items-center gap-4 lg:gap-6 min-w-0">
                    <div class="w-[50px] h-[50px] bg-gray-300 rounded-full border-2 border-white flex items-center justify-center">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex flex-col gap-1 min-w-0">
                            ...
                            <span class="text-base font-bold @if($role->id != 1) text-text-dark @else text-text-light @endif truncate" title="{{ $role->name }}">
                                {{ ucwords($role->name) }}
                            </span>
                        </div>
                    </div>
                    ...
                    <!-- Edit button -->
                    <button @click="showEditModal = true; selectedRole = {{ $role->load('permissions')->toJson() }};" class="w-9 h-9 flex items-center justify-center bg-light-blue rounded-lg hover:bg-blue-200 transition">
            ...
            <div class="bg-white rounded-[24px] shadow-sm p-6 text-center">
                ...
                <p class="text-gray-500 text-sm lg:text-base">No roles found.</p>
            </div>
            @endif

            <!-- 🔹 Keep modal INSIDE this same x-data scope -->
